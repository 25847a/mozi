...
$(function(){
    ...
    var columns = [
        ...
        { data: null,render:function(data, type, full, meta){
        	var html='<button type="button" class="btn  btn-info" onclick="openModal(\''+data.id+'\',\''+data.type+'\')">查看</button>';
        	return html;
        }}
     ]
	getData('table',getParam,'../admin/queryProviderBaseinfoList',columns,function(){
		//表格加载完成事件
  ...
  //审核
    $("#status").click(function(){
    	...
    	if(id.length<1){
    		...
    		if(confirm("按照国家相关规定，是否同意发证")){
    			$.post('../admin/examinePlasma',{"id":id},function(res){
            		if(res.error==-1){
            			alert('审核成功');
            			location.reload();
            		}else{
    ...
    $("#cancel").click(function(){
    	...
    	layer.confirm('是否取消发卡？', function(index){
    		$.post('../admin/cancelExaminePlasma',{"id":id},function(res){
        		if(res.error==-1){
        			alert('审核成功');
        			location.reload();
        		}else{
