...
layui.use(['laydate', 'laypage', 'table', ], function(){
  ...
  table.render({
    elem: '#lossPage'
    ,url: GetURLInfo()+'/queryLossList' //数据接口
    ,where : {
		token : getToken()
	}
    ,page: true //开启分页 
    ...
    {type: 'checkbox', fixed: 'left'}
      ,{field: 'damageOrder', title: '报损订单', sort: true,align:'center'}
	    ...
	    if(layEvent === 'detail') {
	      layer.ready(function() {
	        layer.open({
	          ...
	          maxmin: false,
	          area: ['80%', '80%'],
	          content: GetURLInfo()+'/lossDatelis?damageOrder='+data.damageOrder,
	        /*	btn: ['确定', '取消'],*/
	        });
	      });
	    }
	    });
  table.on('checkbox(lossPage)', function(obj) {
		console.log(obj)
	});
  var $ = layui.jquery, active = {
			update : function() {
				var checkStats = table.checkStatus('lossPage'), data = checkStats.data;
				if(data.length==0){
				...
				layer.ready(function() {
					layer.open({
						...
						area: ['500px', '250px'],
						maxmin: false,            //最大化按钮
						content: GetURLInfo()+'/lossUpdate?damageOrder='+data[0].damageOrder+"&reason="+data[0].reason
					})
				});
			}
		};
...
function doSearch(){
	...
	layui.table.reload('lossPage', {page: {curr:1},where: {"createDate": createDate,"updateDate": updateDate,"damageOrder": damageOrder,"status": status}});
}
function getToken() {
...
! function() {
	$('#add').on('click', function() {
		addWithArea(GetURLInfo()+'/lossAdd',true,['80%', '80%']);
	})
}();
/*日期时间(开始日期、结束日期)*/
