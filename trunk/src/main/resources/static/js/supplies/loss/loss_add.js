...
layui.use(['laydate', 'laypage', 'table', ], function(){
  ...
  table.render({
    elem: '#suppliesInfo'
  ,height:'400px'
    ,url: GetURLInfo()+'/queryLossPage' //数据接口
    ,page: true //开启分页
  ...
  table.render({
	...
	$("#add").click(function(){ 
        ...
        var num = $('#num').val();
        if(num==''){
        	return layer.msg("请输入报损数量");
        }
	   	  var currPageNo=0;
	   	  var 	oldData =  table.cache["addLoss"];
	...
	$("#exactly").click(function(){
		...
		var data =  table.cache["addLoss"];
		var result = JSON.stringify(data);
		$.ajax({
			type : "POST",
			url :GetURLInfo()+"/insertLoss", 
			data:{
				"list":result,
				"reason":reason
			},
