...
layui.use(['laydate', 'laypage', 'table', ], function(){
  ...
  table.render({
    elem: '#suppliesInfo'
  ,height:'400px'
    ,url: GetURLInfo()+'/queryDestroyPage' //数据接口
    ,page: true //开启分页
    ...
    cols: [[ //表头
		...
		{field: 'surplusNumber', title: '剩余数量',align:'center',width:'26%'},
  ...
  table.render({
	...
	$("#add").click(function(){ 
        ...
        var num = $('#num').val();
        if(num==''){
        	return layer.msg("请输入销毁数量");
        }
	   	  var currPageNo=0;
	   	  var 	oldData =  table.cache["addDestroy"];
	...
	$("#exactly").click(function(){
		...
		var data =  table.cache["addDestroy"];
		var result = JSON.stringify(data);
		$.ajax({
			type : "POST",
			url :GetURLInfo()+"/insertDestroy", 
			data:{
				"list":result,
				"reason":reason
			},
