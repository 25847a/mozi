...
function initTable(tableId){
	...
	cols =  [[
		 {field: 'id', title: 'ID',  sort: true,align:'center',display : 'none',minWidth : '0',width : '0',type : "space"/* fixed: 'left'*/}
	      ...
	      ,{field: 'sampleNo', title: '小样号', sort: true,width:'18%',align:'center'}
	      ...
	      ,{field: 'prCreateDate', title: '登记日期',  width:'15%',align:'center', templet:function(d){ return $.myTime.UnixToDate(d.prCreateDate);}}
	      ,{field: 'rechecked', title: '是否复检',width:'14%', align:'center', templet:function(d){ return getBooleanValue(d.rechecked);}}
	]];
	}else{
		cols = [[
			//表头
			 {field: 'id', title: 'ID',  sort: true,align:'center', display : 'none',minWidth : '0',width : '0',type : "space"/* fixed: 'left'*/}
		      ,{field: 'allId', title: '登记号', sort : true, width:'10%', align:'center'}
		      ,{field: 'sampleNo', title: '小样号', sort: true,width:'15%',align:'center'}
		      ,{field: 'providerNo', title: '浆员卡号', width:'15%',align:'center'} 
		      ,{field: 'name', title: '姓名',width:'10%', align:'center'}
		      ,{field: 'sex', title: '性别', width:'10%',align:'center',templet: function (d){ return getSexValue(d.sex);}} 
		      ,{field: 'result', title: '化验结果', width:'10%',align:'center',templet: function (d){ return getCheckResultValue(d.result);}}
		      ,{field: 'hbsag', title: 'HBsAg',width:'10%', align:'center',templet: function (d){ return getCheckedValue(d.hbsag);}}
		      ...
		      ,{field: 'proteinValue',width:'10%', title: '蛋白含量'}
		      ,{field: 'opinion', title: '浆员状态', width:'10%',align:'center',templet: function (d){ return getOpinionValue(d.opinion);}}
		      ,{field: 'day', title: '暂拒天数', width:'10%',align:'center'}
		      ,{field: 'remarks', title: '备注',align:'center'}
		]];
	}
	dataAll(tableId,cols,{"isAssay": "newtest"==tableId?0:1,"type":0,"startTime" : startTime, "token" : getToken()},GetURLInfo() + 'queryListsByCreateDate',GetURLInfo() +'',GetURLInfo() + "",function(){
...
function initTableOnClick(tableId){
	// 表格 tr 单击事件
	var tab = $("#"+tableId).next('.layui-table-view')
			.find('table.layui-table').eq(1);
...
function saveInfo(isAssay){
	...
	if(result == 0){
		...
		var blood = $("#blood").val();
		if(blood==undefined || blood==''){
			var index = parent.layer.alert("还没有做血型的化验!", function() {
				parent.layer.close(index);
			});
			return false;
		}
	...
	var data = {"id":id,"proteinValue":proteinValue,"isAssay":isAssay,"bloodRedProteinValue":bloodRedProteinValue,"blood":blood,"remarks":remarks,"result":result,"reportAdminid":reportAdminid,"suppliesId":suppliesId};
	if(result == 1){
