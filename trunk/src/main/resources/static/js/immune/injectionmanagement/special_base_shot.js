$(function(){
	layui.use(['table'], function(){
		//未注射人员
	  var notShotSpeBase = [[{field : 'id',title : 'id',display : 'none',minWidth : '0',width : '0',type : "space"}
	  ...
	  dataAll("newtest",notShotSpeBase,{"token" : getToken()},"notShotSpeBase",'','',function(){
		  var tab = $("#newtest").next().find('.layui-table tr'); 
	...
	//已注射人员
	  var haveShotSpeBase = [[{field : 'id',title : 'id',display : 'none',minWidth : '0',width : '0',type : "space"}
	  ...
	  dataAll("newtestS",haveShotSpeBase,{"token" : getToken()},"haveShotSpeBase",'','',function(){
	...
	function headTitle(id,updateDate){
		$.ajax({
			url:"/privilegeInjection/specialBaseHead",
			dataType:"json",
			type:"post",
			...
			success:function(result){
				if(result.code == -1){
					...
					$("#shotDate").val(new Date(rd.updateDate).Format("yyyy-MM-dd"));
	...
	$("#shot").click(function(){
		...
		if(id == "" || id == null){
			...
			if((status == "") && (remarks == "" || remarks ==undefined)){
				...
				$.ajax({
					 url:"../privilegeInjection/speBaseToShot",
					type:"post",
					dataType:"json",
	...
	$("#cancelShot").click(function(){
		...
		if(id == "" || id == null){
			layer.alert("请选择浆员进行取消注射");
				...
				$.ajax({
					 url:"/privilegeInjection/speBaseToNotShot",
					type:"post",
					dataType:"json",
