$(function(){
   ...
   var cols=[[ //表头
    	{field: 'providerNo', title: '浆员卡号',align:'center',width:'20%'},
    	{field: 'name', title: '姓名',align:'center',width:'15%'},
    	{field: 'sex', title: '性别',align:'center',width:'15%',templet:function(d){
        	  return getSexValue(d.sex);
      	  }},
    	{field: 'type', title: '类型',align:'center',width:'15%',templet:function(d){
        	  return getPbTypeValue(d.type);
      	  }},
    	{field: 'bloodType', title: '血型',align:'center',width:'15%',templet:function(d){
	 ...
	 dataAllAndWHWithPage("registerNo",cols,{"token" : getToken()},GetURLInfo()+'queryImmuneRegister','','','',true,function(){
			//表格 tr 单击事件
		   var tab = $("#registerNo").next().find('.layui-table tbody tr');
	 ...
	 dataAllAndWHWithPage("registerYes",colsYes,{"token" : getToken()},GetURLInfo()+'queryImmuneRegisterAdopt','','','',true,function(){
			//表格 tr 单击事件
		   var tab = $("#registerYes").next().find('.layui-table tbody tr');
		   tab.click(function(event){
		  	  var tr =$(event.target).closest("tr")[0];
		   	  $("#providerNo").val($(tr).find("td").eq(0).find("div").html());
		   	  $("#name").val($(tr).find("td").eq(1).find("div").html());
...
function doSearch(){
	var startTime = $("#startTime").val();
	var num = $("#num").val();
	layui.table.reload('registerNo', {where: {"startTime": startTime,"num":num}});
	layui.table.reload('registerYes', {where: {"startTime": startTime,"num":num}});
}
function getToken() {
...
function skufansp(){
	...
	if(register==''){
		parent.layer.alert("已登记过,请勿重复登记",function(){
			parent.layer.closeAll();
		});
	return false;
	}
	submission("dengji","登记中.....");//锁住登记按钮,防止重复登记
	 $.ajax({
	        type : "POST",
	        url :GetURLInfo()+"/updateImmuneRegister",
	        data:{
	        	"immuneId":immuneId,
	        ...
	        success : function(result) {
	        	if(result.code==-1){
	        		parent.layer.alert(result.message,function(){
	        			parent.layer.closeAll();
	        			layui.table.reload('registerNo');
	        			layui.table.reload('registerYes');
	        			parent.$("number").val(0);
	        		});
	        	}
 ...
 /*取消登记*/
  function skufanspas(){
	  ...
	  var immuneId = $("#immuneId").val();
		...
		$.ajax({
			type:'post',
			url:GetURLInfo()+'/cancelPuregisterRegister',
