...
public interface HealthMapper {
	...
	List<Chart> selecthealth(Map<String, Object> m);
	 /**
		...
		@Select("<script>" +
				"<if test='service != null and service != \"\" and  service == \"year\" '>" +
					"SELECT TRUNCATE(stepWhen,0) as stepWhen,TRUNCATE(carrieroad,0) as carrieroad,date_format(createtime, '%Y-%m-%d') as date" +
					" FROM healthstep WHERE DATE_FORMAT(createtime, '%Y') = #{timedata} AND userId=#{userId} GROUP BY date_format(createtime,'%Y%m%d')" +
				"</if>" +
				"<if test='service != null and service != \"\" and  service == \"month\" '>" +
				"SELECT TRUNCATE(stepWhen,0) as stepWhen,TRUNCATE(carrieroad,0) as carrieroad,date_format(createtime, '%Y-%m-%d') as date" +
				" FROM healthstep WHERE DATE_FORMAT(createtime, '%Y-%m') = #{timedata} AND userId=#{userId} GROUP BY date_format(createtime, '%Y%m%d')" +
				"</if>" +
				"<if test='service != null and service != \"\" and  service == \"day\" '>" +
				"SELECT TRUNCATE(stepWhen,0) as stepWhen,TRUNCATE(carrieroad,0) as carrieroad, date_format(createtime,'%Y-%m-%d %H:%i:%S') as date"+
				" FROM healthstep WHERE DATE_FORMAT(createtime,'%Y-%m-%d')=#{timedata} AND userId=#{userId} GROUP BY  HOUR(createtime);" +
				"</if>" +
				"<if test='service != null and service != \"\" and  service == \"week\" '>" +
				"SELECT TRUNCATE(stepWhen,0) as stepWhen,TRUNCATE(carrieroad,0) as carrieroad,date_format(createtime, '%Y-%m-%d') as date "+
				 "FROM healthstep WHERE YEARWEEK(DATE_FORMAT(createtime,'%Y-%m-%d'))=YEARWEEK(NOW()) AND userId=#{userId} GROUP BY date_format(createtime, '%Y%m%d')" +
				"</if>" +
			"</script>")
	...
	@Select("<script>" +
			...
			"SELECT heartRate,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y') =#{timedata}"+
		...
		"SELECT tt.*,new,createtime FROM ("+
		"SELECT MAX(heartRate) as max,MIN(heartRate) AS min,COUNT(*) AS count,ROUND(AVG(heartRate)) AS avg,userId FROM ("+
		"SELECT heartRate,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y-%m') =#{timedata}"+
		"  ORDER BY createtime LIMIT 1,99999"+
		") tc"+
		")tt"+
		" INNER JOIN ("+
		...
		"SELECT tt.*,new,createtime FROM ("+
		"SELECT MAX(heartRate) as max,MIN(heartRate) AS min,COUNT(*) AS count,ROUND(AVG(heartRate)) AS avg,userId FROM ("+
		"SELECT heartRate,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y-%m-%d') =#{timedata}"+
		"  ORDER BY createtime LIMIT 1,99999"+
		") tc"+
		")tt"+
		" INNER JOIN ("+
	...
	@Select("<script>" +
			...
			"SELECT highBloodPressure,lowBloodPressure,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y') =#{timedata}"+
			" ORDER BY createtime LIMIT 1,99999"+
			") tc"+
			")tt"+
			" INNER JOIN ("+
		...
		"SELECT tt.*,new,createtime FROM ("+
		"SELECT CONCAT(MAX(highBloodPressure),'/',MAX(lowBloodPressure)) AS max,CONCAT(MIN(highBloodPressure),'/',MIN(lowBloodPressure)) min,COUNT(*) AS count,CONCAT(ROUND(AVG(highBloodPressure)),'/',ROUND(AVG(lowBloodPressure))) as avg,userId FROM("+
		"SELECT highBloodPressure,lowBloodPressure,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y-%m') =#{timedata}"+
		" ORDER BY createtime LIMIT 1,99999"+
		") tc"+
		")tt"+
		" INNER JOIN ("+
		...
		"SELECT tt.*,new,createtime FROM ("+
		"SELECT CONCAT(MAX(highBloodPressure),'/',MAX(lowBloodPressure)) AS max,CONCAT(MIN(highBloodPressure),'/',MIN(lowBloodPressure)) min,COUNT(*) AS count,CONCAT(ROUND(AVG(highBloodPressure)),'/',ROUND(AVG(lowBloodPressure))) as avg,userId FROM("+
		"SELECT highBloodPressure,lowBloodPressure,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y-%m-%d') =#{timedata}"+
		" ORDER BY createtime LIMIT 1,99999"+
		") tc"+
		")tt"+
		" INNER JOIN ("+
	...
	@Select("<script>" +
			...
			"SELECT MAX(stepWhen) AS new,ROUND(AVG(stepWhen)) as avg,ROUND(COUNT(*)*50/100000) as kilometre,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y') =#{timedata}"+
			")tt"+
			" INNER JOIN ("+
			"SELECT tp.userId,date_format(tp.createtime,'%H:%i') as createtime FROM healthstep tp"+
			" INNER JOIN (SELECT MAX(createtime) as createtime FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y') =#{timedata}) tt"+
			" ON tp.createtime=tt.createtime"+
			" WHERE tp.userId=#{userId} AND date_format(tp.createtime,'%Y') =#{timedata}"+
			") tg"+
			" ON tt.userId=tg.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS max,date_format(createtime,'%Y') as maxtime,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y') =#{timedata} GROUP BY date_format(createtime,'%Y-%m') ORDER BY stepWhen DESC  LIMIT 0,1"+
			") tv"+
			" ON tg.userId=tv.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS min,date_format(createtime,'%Y') as mintime,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y') =#{timedata} GROUP BY date_format(createtime,'%Y-%m') ORDER BY stepWhen  LIMIT 0,1"+
			") tj"+
			" ON tg.userId= tj.userId"+
		"</if>" +
		"<if test='service != null and service != \"\" and  service == \"month\" '>" +
			"SELECT tt.new,tg.createtime,tv.max,tv.maxtime,tj.min,tj.mintime,tt.avg,kilometre FROM ("+
			"SELECT MAX(stepWhen) AS new,ROUND(AVG(stepWhen)) as avg,ROUND(COUNT(*)*50/100000) as kilometre,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m') =#{timedata}"+
			")tt"+
			" INNER JOIN ("+
			"SELECT tp.userId,date_format(tp.createtime,'%H:%i') as createtime FROM healthstep tp"+
			" INNER JOIN (SELECT MAX(createtime) as createtime FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m') =#{timedata}) tt"+
			" ON tp.createtime=tt.createtime"+
			" WHERE tp.userId=#{userId} AND date_format(tp.createtime,'%Y-%m') =#{timedata}"+
			") tg"+
			" ON tt.userId=tg.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS max,date_format(createtime,'%Y-%m') as maxtime,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m') =#{timedata} GROUP BY date_format(createtime,'%Y-%m') ORDER BY stepWhen DESC  LIMIT 0,1"+
			") tv"+
			" ON tg.userId=tv.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS min,date_format(createtime,'%Y-%m') as mintime,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m') =#{timedata} GROUP BY date_format(createtime,'%Y-%m') ORDER BY stepWhen  LIMIT 0,1"+
			") tj"+
			" ON tg.userId= tj.userId"+
		"</if>" +
		"<if test='service != null and service != \"\" and  service == \"day\" '>" +
			"SELECT tt.new,tg.createtime,tv.max,tv.maxtime,tj.min,tj.mintime,tt.avg,kilometre FROM ("+
			"SELECT MAX(stepWhen) AS new,ROUND(AVG(stepWhen)) as avg,ROUND(COUNT(*)*50/100000) as kilometre,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m-%d') =#{timedata}"+
			")tt"+
			" INNER JOIN ("+
			"SELECT tp.userId,date_format(tp.createtime,'%H:%i') as createtime FROM healthstep tp"+
			" INNER JOIN (SELECT MAX(createtime) as createtime FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m-%d') =#{timedata}) tt"+
			" ON tp.createtime=tt.createtime"+
			" WHERE tp.userId=#{userId} AND date_format(tp.createtime,'%Y-%m-%d') =#{timedata}"+
			") tg"+
			" ON tt.userId=tg.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS max,date_format(createtime,'%Y-%m-%d') as maxtime,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m-%d') =#{timedata} GROUP BY date_format(createtime,'%Y-%m') ORDER BY stepWhen DESC  LIMIT 0,1"+
			") tv"+
			" ON tg.userId=tv.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS min,date_format(createtime,'%Y-%m-%d') as mintime,userId FROM healthstep WHERE userId=#{userId} AND date_format(createtime,'%Y-%m-%d') =#{timedata} GROUP BY date_format(createtime,'%Y-%m') ORDER BY stepWhen  LIMIT 0,1"+
			") tj"+
			" ON tg.userId= tj.userId"+
		"</if>" +
		"<if test='service != null and service != \"\" and  service == \"week\" '>" +
			"SELECT tt.new,tg.createtime,tv.max,tv.maxtime,tj.min,tj.mintime,tt.avg,kilometre FROM ("+
			"SELECT MAX(stepWhen) AS new,ROUND(AVG(stepWhen)) as avg,ROUND(COUNT(*)*50/100000) as kilometre,userId FROM healthstep WHERE userId=#{userId} AND YEARWEEK(date_format(createtime,'%Y-%m-%d')) =YEARWEEK(NOW())"+
			")tt"+
			" INNER JOIN ("+
			"SELECT tp.userId,date_format(tp.createtime,'%H:%i') as createtime FROM healthstep tp"+
			" INNER JOIN (SELECT MAX(createtime) as createtime FROM healthstep WHERE userId=#{userId} AND YEARWEEK(date_format(createtime,'%Y-%m-%d')) =YEARWEEK(NOW())) tt"+
			" ON tp.createtime=tt.createtime"+
			" WHERE tp.userId=#{userId} AND YEARWEEK(date_format(tp.createtime,'%Y-%m-%d')) =YEARWEEK(NOW())"+
			") tg"+
			" ON tt.userId=tg.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS max,date_format(createtime,'%Y-%m-%d') as maxtime,userId FROM healthstep WHERE userId=#{userId} AND YEARWEEK(date_format(createtime,'%Y-%m-%d')) =YEARWEEK(NOW()) GROUP BY date_format(createtime,'%Y-%m-%d') ORDER BY stepWhen DESC  LIMIT 0,1"+
			") tv"+
			" ON tg.userId=tv.userId"+
			" INNER JOIN (SELECT SUM(stepWhen) AS min,date_format(createtime,'%Y-%m-%d') as mintime,userId FROM healthstep WHERE userId=#{userId} AND YEARWEEK(date_format(createtime,'%Y-%m-%d')) =YEARWEEK(NOW()) GROUP BY date_format(createtime,'%Y-%m-%d') ORDER BY stepWhen  LIMIT 0,1"+
			") tj"+
			" ON tg.userId= tj.userId"+
		"</if>" +
	"</script>")
	...
	@Select("<script>" +
			...
			"SELECT hrv,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y') =#{timedata}"+
			" ORDER BY createtime LIMIT 1,99999"+
			")tc"+
			")tt"+
			" INNER JOIN ("+
		...
		"SELECT tt.*,new,createtime FROM ("+
		"SELECT MAX(hrv) as max,MIN(hrv) AS min,COUNT(*) AS count,ROUND(AVG(hrv)) AS avg,userId from("+
		"SELECT hrv,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y-%m') =#{timedata}"+
		" ORDER BY createtime LIMIT 1,99999"+
		")tc"+
		")tt"+
		" INNER JOIN ("+
		...
		"SELECT tt.*,new,createtime FROM ("+
		"SELECT MAX(hrv) as max,MIN(hrv) AS min,COUNT(*) AS count,ROUND(AVG(hrv)) AS avg,userId from("+
		"SELECT hrv,userId,createtime FROM health WHERE userId=#{userId} AND date_format(createtime,'%Y-%m-%d') =#{timedata}"+
		" ORDER BY createtime LIMIT 1,99999"+
		")tc"+
		")tt"+
		" INNER JOIN ("+
