$(function(){
  ...
  var  cols= [[ //表头
     ...
     {field: 'num', title: '基础针数',align:'center'},
  ...
  dataAllIsOpenLimit("basics",cols,{"token" : getToken()},GetURLInfo() + 'basicRulesList',GetURLInfo() +'basicRulesUpdate',GetURLInfo() + "deleteBasicRules",true,'',10);
...
function settingList(url,data){
	...
	function doSearch(){
		...
		layui.table.reload('basics', {where: {
			"immuneId": immuneId
			}});
	}
	...
	function getToken() {
	 ...
	 ! function() {
		 	...
		 	$('#immAdd').on('click', function() {
		 		addWithArea(GetURLInfo() +'basicRulesAddPage',true,['400px', '280px']);
		 		}
		 	)
		 }();
		 /**
