  ...
  table.render({
    ...
    cols: [[ //表头
     ...
     {field: 'id', title: '编号', sort: true,align:'center', fixed: 'left'}
      ...
      ,{field: 'destroyOrder', title: '销毁订单',width:'15%', sort: true,align:'center'}
      ,{field: 'reason', title: '销毁原因',width:'18%', align:'center'}
      ...
      ,{field: 'createDate', title: '销毁时间',width:'17%', align:'center',templet: function (d){
	  ...
	  $('.hcj').on('click', function() {
		...
		function examination(data, num, sun, result, func) {
			if (data[0].status != num) {
				...
				var dd={"destroyOrder":data[0].destroyOrder,"status":data[0].status};
				$.post(GetURLInfo()+"/updateDestroyStatus",dd,function(result){
					func(result);
				})
			}
		};
...
function doSearch(){
	...
	layui.table.reload('returnPage', {page: {curr:1},where: {
		...
		"destroyOrder": destroyOrder,"status": status}});
}
function getToken() {
